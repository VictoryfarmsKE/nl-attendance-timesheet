...
def add_attendance_data(payroll_entry):
    ...
    for entry in salary_slips:
        frappe.enqueue(
            process_salary_slip,
            salary_slip_name=entry.get('name'),
            queue='long',  # Use 'default' or 'long' based on your requirement
        )
def process_salary_slip(salary_slip_name):
    leave_type_data = frappe._dict()

    shift_data = frappe._dict()
    salary_slip = frappe.get_doc('Salary Slip', salary_slip_name.get('name'))
